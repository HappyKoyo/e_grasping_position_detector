#include "e_grasping_position_detector/grasping_position_detector.h"

class GraspingPositionDetector{
private:
...
public:
  ...
  void ImageRangeCB(const e_object_recognizer::ImageRange input){
  ...
  void CalcCentroid(const sensor_msgs::PointCloud2::Ptr &input){
    ...
    std::cout << ave_x <<" "<<ave_y<<" "<<ave_z<<" "<<sum_point<<std::endl;
    ...
    double THETA = -1*M_PI/6;
    double REALSENSE_HEIGHT = 1.20;
...
int main (int argc, char** argv){
  ros::init(argc, argv, "grasping_position_detector");
  ros::NodeHandle nh("~");
  
  GraspingPositionDetector* range_to_centroid = new GraspingPositionDetector(nh);
  ros::spin();
}
